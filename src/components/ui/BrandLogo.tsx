...
/** Monogram only: brand-yellow "e", brand-white "d" — exactly as in the official logo. */
export function Monogram({ className, title }: { className?: string; title?: string }) {
  return (
    ...
    >
      ...
      <path fillRule="evenodd" fill="var(--color-brand-yellow)" d={PATH_E} />
      <path fill="var(--color-brand-white)" d={PATH_D} />
    </svg>
  );
}
...
export function BrandLockup({ className, compact = false }: { className?: string; compact?: boolean }) {
  return (
    <span className={cn("inline-flex items-center gap-3 text-fg", className)}>
      ...
      <span aria-hidden className="h-7 w-px bg-brand-yellow" />
  ...
  return (
    <span className={cn("inline-flex gap-4 font-display text-xs tracking-[0.3em] uppercase", className)}>
      {brand.tagline.map((word) => (
        <span key={word}>
          <span className="text-[1.25em] leading-none text-brand-yellow lowercase">{word[0]}</span>
          {word.slice(1)}
        </span>
      ))}
    </span>
